var HandleKegiatan = function () {
    var initTable = function () {
        ...
        var oTable = table.dataTable({
            ...
            ajax: {
                url: BASE_URL + "master/kegiatan/kegiatan_request_list", // ajax source
                type: 'POST'
            },
            lengthMenu: [
                [5, 10, 20, -1],
		...
		$('#Negara').on('change', function(){
            ...
            $.when($.post(BASE_URL + 'master/kegiatan/get_kota', {id_negara: id_negara}, function (data) {
                ...
                temp_tujuan = $('#temp_tujuan').val();
                $('#Tujuan').val($('#Tujuan_def').val()).trigger('change');

                // $("#Tujuan").val(temp_tujuan);
            });
        });
        ...
        $('#simpan').on('click',function(){
            ...
            var add ={ID: $('#ID').val(), NamaKegiatan: $('#NamaKegiatan').val(), JenisKegiatan: $('#JenisKegiatan').val(), Negara: $('#Negara').val(),
                 Tujuan: $('#Tujuan').val(), StartDate: $('#StartDate').val(), EndDate: $('#EndDate').val(), Penyelenggara: $('#Penyelenggara').val(), method:$('#method').val()};
            console.log(add);
            $.ajax({
                url : BASE_URL+"master/kegiatan/kegiatan_request_update",
                type: "POST",
                dataType: 'JSON',
                data: add,
                    ...
                    if(data.status === true){
                         $('#modal_new_kegiatan').modal('hide');
                            window.setTimeout(function() {
                                ...
                                window.location.href = BASE_URL + "master/kegiatan/kegiatan_request";
                            }, 200);
                    }
                    else
                    {
                    ...
                    buttons: {
                      success: {
                        ...
                        callback: function() {
                            ...
                            $.ajax({
                                url : BASE_URL+"master/kegiatan/kegiatan_delete",
                                type: "POST",
                                dataType: 'JSON',
                                data: {ID : aData[0]},
                                success: function(data)
        ...
        table.on('click', '#edit_kegiatan', function(e) {
            ...
            $.ajax({
                url : BASE_URL+"master/kegiatan/get_data_kegiatan",
                type: "POST",
                dataType: 'JSON',
                data: {ID : aData[0]},
                success: function(data)
...
if (App.isAngularJsApp() === false) {
    jQuery(document).ready(function() {
        HandleKegiatan.init();
        $('.baru').attr('readonly', true);
        $('#JenisKegiatan').val($('#JenisKegiatan_def').val()).trigger('change');
        $('#Negara').val($('#Negara_def').val()).trigger('change');
    });
}